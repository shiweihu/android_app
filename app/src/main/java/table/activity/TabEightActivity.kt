...
class TabEightActivity : Activity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var titleText = myApp.menuListL?.get(7)?.title
    ...
    override fun onStart() {
        super.onStart()
        var myApp = this.application as MyApplication
        val TabEight = myApp.menuListL?.get(7) as menuItem
        var tip = TabEight?.tips
        var content = TabEight?.content
        var title = TabEight?.title
        title = title?.replace("\n", "")
        ...
        listView.adapter = TabEightActivity.ListItemAdapt(this, TabEight)
